...
\title{Take A Relaxing Bath} \date{\today{}}
\maketitle
...
\item Figure out the influence of parameters such as the shape of the bathtub or the motions made by the bather or so.
\item Propose a strategy to keep the temperature as even as possible.
\end{itemize}
In the first step, we build the simplest model. The shape of the bathtub is a cuboid, and the bather will stay still. In the second step, the bather moves slowly and the shape of the bathtub changes. In the third step, we develop the conclusion of the strategy.
...
\begin{tabular}{l|l}
  ...
  ${\gamma}_2$   &Heat exchange coefficient between water and bather \\
...
\begin{tabular}{l|l}
  ...
  $u_2$          &Temperature of bather's surface \\
...
\begin{itemize}
\item \textbf{assumption 1} The surface temperature of bathtub and the bather keeps constant.
...
\item \textbf{assumption 5} Water, bathtub, bather and the surrounding air compose a closed heat system.
\end{itemize}

% the main section of the paper
\section{PDE Model for Heating Water}
...
exchange between water and the bather, heat exchange between water and the bathtub, heat energy of hot water from the
...
As shown in Figure 1, heat of water in the bathtub exchanges with air, the bather and the bathtub.
...
\end{equation}
in which $\Delta Q_1$ is the heat transferred from air, bather and bathtub,
...
The boundary of water consists of four parts: water-air boundary, water-bather boundary, water-bathtub boundary, and the square for faucet. We denote them as
...
\end{equation}
In the equation, $u_2$ is the surface temperature of the bather. We assume it is constant. $u_4$ is the temperature of hot water.
...
Since our equations are complex and the region is quite irregular, it's impossible to get an analytical solution. Finite element method is normally adopted to deal with this kind of problem.
...
and the initial value condition is $35\,^{\circ}\mathrm{C}$.
 We set 120s as time period. The following graphs show the results we get.
...
\subsection{Parameters of Bather}
\label{sec:parameters of bather}
\begin{itemize}
